...
def copy_layers_over(rng, num_layers, trained_params, prev_activations):

    layers, activations = create_layers(rng, num_layers, prev_activations)
    ...
    for layer, value in trained_params.items():
        if layer in params['params']:
            ...
            if 'bias' in value:
                copy_arr = jax.device_get(params['params'][layer]['bias']).copy()
                for num_node_i in range(len(value['bias'].T)):
                    copy_arr[num_node_i] = jax.device_get(value['bias']).T[num_node_i]
...
def add_new_node(rng, num_layers, trained_params, prev_activations):
    if len(num_layers) == 1:
        ...
        random_element = jax.random.choice(inp_rng, jnp.asarray(num_layers)[:-1]).item()
...
def remove_layers_over(rng, num_layers, trained_params, prev_activations):
    layers, activations = create_layers(rng, num_layers, prev_activations)
    model = GenomeClassifier(layers=layers, activations=activations)
